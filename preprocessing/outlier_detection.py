...
class OutlierDetector:
    ...
    def clean(self, df):
        ...
        # Identify joint pairs by finding every column ending with '_x' and its corresponding '_y'.
        joint_pairs = []
        for xcol in data.columns:
            if xcol.endswith('_x'):
                base = xcol[:-2]
        ...
        rolling_outliers = self.detect_outliers_rolling(data, joint_pairs,
                                                        ...
                                                        self.rolling_k)
        outlier_indices = diff_outliers.union(rolling_outliers)
        print("Outlier frame indices:", outlier_indices)
        ...
        # Also set zeros (detection failures) to NaN.
        for (xcol, ycol) in joint_pairs:
            data.loc[data[xcol] == 0, xcol] = np.nan
            data.loc[data[ycol] == 0, ycol] = np.nan
        ...
        # Final filling in case any NaNs remain.
        data = data.bfill().ffill().fillna(0)
        return data
    ...
    def int_zero(self, data):
        """
        Interpolate over keypoint detection errors that result in (0,0) coordinates.
        
        Many detection models (such as OpenPose) sometimes output (0,0) when they fail to
        detect a keypoint reliably. This method replaces zero values in coordinate columns 
        (assumed to be those ending in '_x' or '_y') with NaN, and then uses linear interpolation 
        to estimate the missing values. This is especially useful when there are multiple consecutive 
        zero detections, where simply deleting one frame would lead to a larger error.
        
        Parameters:
            data : pd.DataFrame
                Input DataFrame containing keypoint coordinate columns.
        
        Returns:
            A new DataFrame with zeros replaced by interpolated values.
        """
        data_int = data.copy()
        # Identify coordinate columns.
        coordinate_cols = [col for col in data_int.columns if col.endswith('_x') or col.endswith('_y')]
        for col in coordinate_cols:
            # Replace zero values with NaN.
            data_int[col] = data_int[col].replace(0, np.nan)
            # Use linear interpolation in both directions.
            data_int[col] = data_int[col].interpolate(method='linear', limit_direction='both')
        return data_int
